...
- **Datetime format in DB** — `start_datetime`, `fix_datetime`, `end_datetime` stored as `mm-dd-yyyy hh:mm` (e.g. `07-03-2020 02:53`). The `month` column stored as `mmm-yy` (e.g. `Jan-26`). Written via `getDateTimeValue()` in `app_edited.js` and `getDateTimeInputValue()` in `repair_info_edited.js`; parsed back via `parseDateTime()` (month-first) in `repair_info_edited.js` / `kpi_dashboard_edited.js` / `parseSimpleDateTime()` in `app_edited.js`, all of which handle both 2-digit and 4-digit years. **User-facing display** uses `mm/dd/yyyy hh:mm` (`formatDateTime()` in `repair_info_edited.js`); date-only displays (PM due dates in `pm_edited.js`, red-tag dates in `red_tag_edited.js`) show as `mm/dd/yyyy`. Date/time `<input>` values stay in browser format (`YYYY-MM-DD` / `HH:mm`).
